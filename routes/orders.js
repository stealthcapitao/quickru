...
var accept = require("../models/accept");
...
router.get("/:id/accept", function(req, res){
  order.findById(req.params.id).populate("order").exec(function(err, foundorder){
      if(err){
          console.log(err);
      } else {
          res.render("orders/accept", {order: foundorder});
      }
  });
});
...
router.post("/:id/accept", middleware.isLoggedIn, function(req, res){
  ...
  var order = req.params.id;
    ...
    var newaccept = {name: name, image: image, description: desc, author:author, order:order};
    // Create a new order and save to DB
    accept.create(newaccept, function(err, newlyCreated){
        if(err){
            console.log(err);
        } else {
            //redirect back to orders page
            req.flash("success","Вы подали заявку. Заказчик свяжется с вами");
            res.redirect("/orders");
        }
    });
  });

  router.get("/:id/accepted", function(req, res){
    if(req.query.search && req.xhr) {
        const regex = new RegExp(escapeRegex(req.query.search), 'gi');
        // Get all orders from DB
        accept.find({name: regex}, function(err, allaccepts){
           if(err){
              console.log(err);
           } else {
              res.status(200).json(allaccepts);
           }
        });
    } else {
        // Get all orders from DB
        accept.find({order:req.params.id}, function(err, allaccepts){
           if(err){
               console.log(err);
           } else {
              if(req.xhr) {
                ...
                res.render("orders/accepted",{accepts: allaccepts, page: 'orders'});
              }
           }
        });
    }
